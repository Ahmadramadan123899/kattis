...
def solve_puzzle(lines: list[str]) -> int:
    ...
    Args:
        lines (list[str]): The input will consist of exactly 2 lines, each containing exactly
                        2 characters.
                        Each character is either a number 1 through 3 (representing one of the tiles) 
                        or a dash (-) (the empty space).
                        The puzzle state represented by the input is guaranteed to be a solvable configuration.

    Returns: Output a singe integer, indicating the minimum number of moves required to solve the puzzle
             from the provided starting position, or
